from flask import Flask, request, render_template_string, send_file, url_for
import pandas as pd
import os
from datetime import datetime

app = Flask(__name__)

UPLOAD_DIR = "/tmp"
os.makedirs(UPLOAD_DIR, exist_ok=True)

# ================= HTML =================
HTML = """
<!doctype html>
<html lang="fr">
<head>
<meta charset="utf-8">
<title>Boat Data Analyzer</title>
<link href="https://cdn.jsdelivr.net/npm/bootstrap@5.3.2/dist/css/bootstrap.min.css" rel="stylesheet">
</head>

<body class="p-4 bg-dark text-light">
<div class="container">

<h1 class="text-center mb-4">Boat Data Analyzer</h1>

<form method="post" action="/upload" enctype="multipart/form-data">
<input class="form-control mb-3" type="file" name="file" required>
<button class="btn btn-primary">Analyser</button>
</form>

{% if table %}
<hr class="my-5">

<h2 class="text-center {{ 'text-danger' if cheat else 'text-success' }}">
{{ etat }}
</h2>

<div class="text-center mb-4">
<b>Embarcation :</b> {{ boat }} |
<b>Date :</b> {{ date }} |
<b>Heure :</b> {{ time }}
</div>

<div class="d-flex justify-content-center mb-4">
<a class="btn btn-success" href="{{ download }}">Télécharger CSV</a>
</div>

<div class="table-responsive">
{{ table|safe }}
</div>

{% endif %}

</div>
</body>
</html>
"""

# ================= CSV ROBUSTE =================
def load_link_csv(file):

    raw = pd.read_csv(
        file,
        header=None,
        sep=",",
        engine="python",
        on_bad_lines="skip"
    )

    # extraction meta robuste
    boat = str(raw.iloc[2,1]) if len(raw) > 2 else "N/A"
    date = str(raw.iloc[3,1]) if len(raw) > 3 else "N/A"
    time = str(raw.iloc[4,1]) if len(raw) > 4 else "N/A"

    header = raw.iloc[19]
    df = raw.iloc[22:].copy()
    df.columns = header
    df = df.reset_index(drop=True)

    return df, boat, date, time

# ================= ANALYSE =================
def analyze_dataframe(df):

    df = df.copy()

    df["Time"] = pd.to_numeric(df.get("Section Time"), errors="coerce")
    df["TPS"] = pd.to_numeric(df.get("TPS (Main)"), errors="coerce")
    df["Lambda"] = pd.to_numeric(df.get("Lambda 1"), errors="coerce")
    df["Fuel"] = pd.to_numeric(df.get("Fuel Pressure"), errors="coerce")

    df = df.dropna(subset=["Time","TPS","Lambda","Fuel"])
    df = df[df["Time"].diff().fillna(0) >= 0]

    # ===== REGLE HRL =====
    df["OUT_FUEL"] = df["Fuel"] < 55

    cheat = df["OUT_FUEL"].any()

    if cheat:
        cheat_time = df[df["OUT_FUEL"]].iloc[0]["Time"]
    else:
        cheat_time = None

    return df, cheat, cheat_time

# ================= ROUTES =================
@app.route("/")
def index():
    return render_template_string(
        HTML,
        table=None
    )

@app.route("/upload", methods=["POST"])
def upload():

    file = request.files["file"]

    df, boat, date, time = load_link_csv(file)
    df, cheat, cheat_time = analyze_dataframe(df)

    if cheat:
        etat = "Datalog NOT compliant with rules"
    else:
        etat = "PASS"

    fname = f"result_{datetime.now().timestamp()}.csv"
    path = os.path.join(UPLOAD_DIR, fname)
    df.to_csv(path, index=False)

    table = df.head(200).to_html(
        classes="table table-dark table-striped",
        index=False
    )

    return render_template_string(
        HTML,
        table=table,
        cheat=cheat,
        etat=etat,
        boat=boat,
        date=date,
        time=time,
        download=url_for("download", fname=fname)
    )

@app.route("/download")
def download():
    return send_file(
        os.path.join(UPLOAD_DIR, request.args["fname"]),
        as_attachment=True
    )

# ================= RENDER =================
if __name__ == "__main__":
    port = int(os.environ.get("PORT", "5000"))
    app.run(host="0.0.0.0", port=port)

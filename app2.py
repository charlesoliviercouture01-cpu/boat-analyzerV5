...
<div class="table-responsive mb-5">{{ table|safe }}</div>
{% endif %}
...
def load_csv_robuste(file):
    ...
    # trouver ligne header automatiquement
    header_row = None
    for i in range(30):
...
def analyze_dataframe(df):
    ...
    col_map = {
        "Section Time": ["section time", "time"],
        "TPS": ["tps"],
        "AFR": ["lambda"],
        "Fuel": ["fuel"],
        "ECT": ["ect"]
    ...
    df["Time"] = pd.to_numeric(df[cols["Section Time"]], errors="coerce")
